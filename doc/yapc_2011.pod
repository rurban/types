...
C<< my TYPE $i; my My::Class $o; >> <br>
C<< Declaration only. package TYPE must exist >>

=item Artur Bergman's B<types> module

C<< Declare and check type safety. int, double, number, string. 
params and return values. >>

=item Optimizations planned for 5.16

C<< smaller and faster, ensured at compile-time. 
...
B<rurban> active in p5p since 1995.

maintains B<cygwin perl> since 5.8.8 
and 3-4 modules: illguts, B<B::*> => 5.10, 
mainly the "compiler".

In the future a lot more time for the compiler and CORE.

I'm known as the BAD guy on p5p who started fighting with Nick.
...
<small>Note: C<@a> is declared as C<int>, but not blessed.</small>
...
Perfect hashes - I<guaranteed O(1) lookup, dynamic hash function generation>
...
C<:const> hashes are always C<:perfect>
...
or C<package Class> B<:locked> or in Moose immutable.
